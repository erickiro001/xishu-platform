...
import { submitDemand } from '@/lib/services';
...
export default function DemandFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !form.demandContent.trim() ||
      !form.contactName.trim() ||
      !form.phone.trim()
    ) {
      setFormError('请填写企业名称、需求信息、联系人和手机号码');
      return;
    }
    ...
    try {
      await submitDemand({
        ...
        requirement: form.demandContent.trim(),
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <form onSubmit={handleSubmit} className="px-4 pt-16 md:pt-32 pb-8 space-y-5 desktop-container">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            需求信息 <span className="text-red-500">*</span>
          </label>
          <textarea
            name="demandContent"
            value={form.demandContent}
            onChange={handleChange}
            placeholder="请描述您的业务需求（尽量1-2句话）"
            required
            rows={4}
        ...
        <div>
          ...
          >
            ...
            <span className="text-xs text-gray-400">点击上传相关附件（文档/图片，单个文件≤100MB）</span>
          </button>
          {files.length > 0 && (
